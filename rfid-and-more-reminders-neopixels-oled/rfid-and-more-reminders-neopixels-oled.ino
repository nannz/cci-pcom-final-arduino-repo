...
//implemented OLED SH1106 in this sketch, to show the category and the time left.
...
void loop() {
  ...
  //========================
  pixels.clear();//clear the neopixel
  ...
  if (int(id.uidByte[0]) == card3[0] && int(id.uidByte[1]) == card3[1] && int(id.uidByte[2]) == card3[2] && int(id.uidByte[3]) == card3[3]) {
    ...
    //display.setTextColor(WHITE);
    //display.setCursor(0, 0);
    //display.println("SEAFOOD");
    //display.display();
  }

  //print the new card on serial monitor with its hex value.
  Serial.print("NewCard ");
  PrintHex(id.uidByte, id.size);
  Serial.println("");
  
 //************************************************
  //try to print something on the OLED screen.
  uidString = String(id.uidByte[0]) + " " + String(id.uidByte[1]) + " " + String(id.uidByte[2]) + " " + String(id.uidByte[3]);
